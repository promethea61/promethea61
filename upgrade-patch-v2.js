// ============================================================
// PROMETHEA UPGRADE PATCH — v2 (video ada suara + Drive support)
// ============================================================
...
(function() {
  ...
  // ==========================================================
  // 1. MEMORIES HOVER — parallax zoom + Ken Burns slow zoom
  // ==========================================================

  const MEM_STYLE = `
    .mem-card { overflow: hidden; }
    .mem-img { overflow: hidden; position: relative; }

    /* Ken Burns — foto bergerak zoom in perlahan saat card ditampilkan */
    @keyframes kenBurns {
    ...
    /* Hover: percepat sedikit dan zoom lebih */
    ...
    /* Stagger Ken Burns — tiap card mulai di fase berbeda biar tidak sync */
    #memoriesGrid .mem-card:nth-child(1)  .mem-img img { animation-delay: 0s; }
    #memoriesGrid .mem-card:nth-child(2)  .mem-img img { animation-delay: -1.3s; }
    #memoriesGrid .mem-card:nth-child(3)  .mem-img img { animation-delay: -2.6s; }
    #memoriesGrid .mem-card:nth-child(4)  .mem-img img { animation-delay: -3.9s; }
    #memoriesGrid .mem-card:nth-child(5)  .mem-img img { animation-delay: -5.2s; }
    #memoriesGrid .mem-card:nth-child(6)  .mem-img img { animation-delay: -6.5s; }
    #memoriesGrid .mem-card:nth-child(7)  .mem-img img { animation-delay: -0.7s; }
    #memoriesGrid .mem-card:nth-child(8)  .mem-img img { animation-delay: -2.0s; }
    #memoriesGrid .mem-card:nth-child(9)  .mem-img img { animation-delay: -4.3s; }
    #memoriesGrid .mem-card:nth-child(10) .mem-img img { animation-delay: -5.6s; }
    #memoriesGrid .mem-card:nth-child(11) .mem-img img { animation-delay: -1.0s; }
    #memoriesGrid .mem-card:nth-child(12) .mem-img img { animation-delay: -3.3s; }
  `;
  ...
  function patchMemoryCards() {
    document.querySelectorAll('#memoriesGrid .mem-card').forEach((card, i) => {
      ...
      let coverImg = imgWrap.querySelector('img');
      ...
      if (!coverImg && imgSrc) {
        ...
        coverImg.style.cssText = 'position:absolute;inset:0;width:100%;height:100%;object-fit:cover;border-radius:inherit;';
  ...
  // ==========================================================
  // 2. SONG CANVAS POPUP — member photos cycle
  // ==========================================================
  ...
  const CANVAS_STYLE = `
    ...
    #songCanvas .sc-pb.active .sc-pb-fill {
      animation: scBarAnim var(--sc-dur, 4s) linear forwards;
    }
    ...
    @media(max-width:768px) {
      #songCanvas { right: 12px; width: 160px; bottom: calc(90px + 12px); }
      #songCanvas .sc-frame { width: 160px; height: 240px; }
    }
  `;
    ...
    <div class="sc-frame">
      ...
      <div class="sc-info">
        ...
        <div class="sc-class">
          ...
          <span id="scClass">Promethea</span>
        </div>
      </div>
  ...
  function scShow(idx) {
    ...
    document.querySelectorAll('#scProgress .sc-pb').forEach((pb, i) => {
      ...
      if (i === idx) {
        ...
        fill.style.setProperty('--sc-dur', scDuration + 'ms');
      }
    });
  ...
  function scOpen(trackIdx) {
    ...
    scDismissed = false;
    scCurrentTrack = trackIdx;
    const t = (typeof tracks !== 'undefined') ? tracks[trackIdx] : null;
    if (t) {
      document.getElementById('scSong').textContent = t.name;
      document.getElementById('scClass').textContent = t.album;
    }
  ...
  if (audioEl) {
    audioEl.addEventListener('play', () => {
      if (scCurrentTrack >= 0 && !scDismissed) scOpen(scCurrentTrack);
    });
    audioEl.addEventListener('pause', () => {
      const fills = document.querySelectorAll('#scProgress .sc-pb.active .sc-pb-fill');
      fills.forEach(f => f.style.animationPlayState = 'paused');
      clearTimeout(scTimer);
    });
  }

  console.log('[Promethea Upgrade v2] Ken Burns + hover + Song Canvas loaded ✓');
})();
